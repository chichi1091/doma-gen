...
public class Gen extends AbstractTask {
   ...
   *
   * @param driverClassName {@link Driver} のサブタイプのクラス名
   */
   ...
   *
   * @param genDialectClassName {@link GenDialect} のサブタイプのクラス名
   */
   ...
   *
   * @param schemaName 対象とするテーブルが属するスキーマ名
   */
   ...
   *
   * @param ignoredTableNamePattern 対象としないテーブル名の正規表現
   */
   ...
   *
   * @param tableTypes カンマまたは空白で区切られたテーブルの型のリスト
   */
  public void setTableTypes(String tableTypes) {
    if (tableTypes != null && tableTypes.length() > 0) {
      StringTokenizer tokenizer = new StringTokenizer(tableTypes, ",", false);
   ...
   *
   * @param templatePrimaryDir テンプレートを格納するプライマリディレクトリ、使用しない場合 {@code null}
   */
  ...
  protected void doPrepare() {
    if (dialectName == null && genDialectClassName == null) {
      ...
      if (name == null) {
        throw new GenException(Message.DOMAGEN0022, "url", "dialectName");
      }
    ...
    if (driverClassName == null) {
      ...
      if (driverClassName == null) {
        throw new GenException(Message.DOMAGEN0022, "url", "driverClassName");
      }
    }
    ...
    Logger.info(Message.DOMAGEN0017.getMessage(dialect.getClass().getName()));
  ...
  protected TableMetaReader createTableMetaReader() {
    return globalFactory.createTableMetaReader(
  ...
  protected EntityPropertyClassNameResolver createEntityPropertyClassNameResolver() {
    return globalFactory.createEntityPropertyClassNameResolver(
        entityConfig.getEntityPropertyClassNamesFile());
  }
  ...
  protected EntityPropertyDescFactory createEntityPropertyDescFactory() {
    return globalFactory.createEntityPropertyDescFactory(
        dialect,
        entityPropertyClassNameResolver,
        versionColumnNamePattern,
        entityConfig.getGenerationType() == null
            ? null
            : entityConfig.getGenerationType().convertToEnum(),
        entityConfig.getInitialValue(),
        entityConfig.getAllocationSize(),
  ...
  protected EntityListenerDescFactory createEntityListenerDescFactory() {
    return globalFactory.createEntityListenerDescFactory(
        entityConfig.getPackageName(), entityConfig.getListenerSuperclassName());
  }
  ...
  protected DaoDescFactory createDaoDescFactory() {
    return globalFactory.createDaoDescFactory(
        daoConfig.getPackageName(), daoConfig.getSuffix(), daoConfig.getConfigClassName());
  }
  ...
  protected SqlTestCaseDescFactory createSqlTestCaseDescFactory() {
    return globalFactory.createSqlTestCaseDescFactory(
  ...
  protected SqlTestSuiteDescFactory createSqlTestSuiteDescFactory() {
    return globalFactory.createSqlTestSuiteDescFactory(sqlTestCaseDescFactory);
  }
  ...
  protected void processAll() {
      ...
      for (TableMeta tableMeta : tableMetas) {
        EntityDesc entityDesc =
            entityDescFactory.createEntityDesc(
                tableMeta, entityConfig.getEntityPrefix(), entityConfig.getEntitySuffix());
    ...
    if (sqlTestCaseConfig.isGenerate()) {
      SqlTestSuiteDesc sqlTestSuiteDesc =
          sqlTestSuiteDescFactory.createSqlTestSuiteDesc(sqlTestCaseConfig.getSqlFiles());
      sqlTestSuiteDesc.getTestCaseDescs().forEach(this::generateSqlTest);
    }
  }

  protected void processSingleEntity() {
    ResultSetMetaReader reader = new ResultSetMetaReader(dialect, dataSource);
  ...
  protected void generateEntityListener(EntityListenerDesc entityListenerDesc) {
    ...
    GenerationContext context =
        new GenerationContext(
            ...
            entityConfig.getEncoding(),
            entityConfig.isOverwriteListener());
    generator.generate(context);
  }
  ...
  protected void generateSql(DaoDesc daoDesc, SqlDesc sqlDesc) {
    File sqlFile =
        FileUtil.createSqlDir(
            sqlConfig.getDestDir(), daoDesc.getQualifiedName(), sqlDesc.getFileName());
    GenerationContext context =
        new GenerationContext(
            sqlDesc, sqlFile, sqlDesc.getTemplateName(), "UTF-8", sqlConfig.isOverwrite());
    generator.generate(context);
  }
   ...
   * SQLのテストコードを生成します。
   *
   * @param sqlTestCaseDesc SQLテスト記述
   */
  protected void generateSqlTest(SqlTestCaseDesc sqlTestCaseDesc) {
    ...
    GenerationContext context =
        new GenerationContext(
            sqlTestCaseDesc,
            javaFile,
            sqlTestCaseDesc.getTemplateName(),
