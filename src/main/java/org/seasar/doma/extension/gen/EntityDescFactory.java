...
public class EntityDescFactory {
   ...
   * @param showDbComment コメントを表示する場合 {@code true}
   ...
   *
   * @param dest エンティティプロパティ記述
   * @param src エンティティプロパティ情報
   * @since 1.7.0
   */
  ...
  protected Set<EntityPropertyInfo> getSuperclassEntityPropertyInfo() {
    ...
    for (Class<?> clazz = superclass; clazz != Object.class; clazz = clazz.getSuperclass()) {
      ...
      for (Field field : clazz.getDeclaredFields()) {
        ...
        propertyInfo.generatedValue = field.getAnnotation(GeneratedValue.class);
        propertyInfo.sequenceGenerator = field.getAnnotation(SequenceGenerator.class);
        propertyInfo.tableGenerator = field.getAnnotation(TableGenerator.class);
   ...
   * エンティティプロパティ情報です。
   *
   * <p>データベースではなくエンティティクラスから得られた情報です。
   *
   * @author taedium
   * @since 1.7.0
   */
