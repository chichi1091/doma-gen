...
public class GlobalFactory {
   ...
   * @param tableNamePattern 読み取り対象とするテーブル名のパターン
   * @param ignoredTableNamePattern 読み取り非対象とするテーブル名のパターン
   * @param tableTypes テーブルの型のリスト
   ...
   */
  public ResultSetMetaReader createResultSetMetaReader(GenDialect dialect, DataSource dataSource) {
   ...
   *
   * @param dialect 方言
   * @param propertyClassNameResolver プロパティクラス名のリゾルバ
   * @param versionColumnNamePattern バージョンカラム名パターン
   * @param generationType 識別子を生成する方法
   ...
   *
   * @param dialectClassName {@code org.seasar.doma.jdbc.dialect.Dialect}のサブクラスの名前
   * @param driverClassName {@link Driver} のサブクラスの名前
   * @param url JDBC接続URL
  ...
  public SqlTestCaseDescFactory createSqlTestCaseDescFactory(
      String dialectClassName, String driverClassName, String url, String user, String password) {
    return new SqlTestCaseDescFactory(dialectClassName, driverClassName, url, user, password);
  }

  /**
   * SQLテストスイート記述ファクトリを作成します。
   *
   * @param sqlTestCaseDescFactory SQLテストケース記述ファクトリ
   ...
   * @since 1.11.0
   */
  public SqlDescFactory createSqlDescFactory(File templatePrimaryDir, GenDialect dialect) {
